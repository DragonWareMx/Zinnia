<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class ImageSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        //
        DB::table('images')->insert([
            'link' => 'Enlight259.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight260.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight261.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight262.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight263.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight265.PNG',
            'descripcion'=>'',
            'production_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'CARTELGENERAL.png',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'CARTELSINDATOS.JPG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight165.PNG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight170.PNG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'FB_IMG_1548814856338.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'FB_IMG_1552885888276.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'FB_IMG_1572119939028.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'Foto2LaPrudencia.png',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0457.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0327.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0292.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0272.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0110.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0985.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0079.JPG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0050.JPG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0012.JPG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight177.JPG',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0552.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0484.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0478.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'LaPrudenciaFaustoJijonQuelal-6.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'Foto3LaPrudencia.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'LaPrudenciaNinaFaustoJijonQuelal.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'LaPrudenciaFaustoJijonQuelal.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '6.jpg',
            'descripcion'=>'',
            'production_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'FotodeProcella.jpg',
            'descripcion'=>'',
            'event_id'=>'2',
        ]);
        DB::table('images')->insert([
            'link' => 'ProximamenteSalavirtual.PNG',
            'descripcion'=>'',
            'event_id'=>'3',
        ]);
        DB::table('images')->insert([
            'link' => 'Enlight271.PNG',
            'descripcion'=>'',
            'event_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'FotodeProcella(1).jpg',
            'descripcion'=>'',
            'event_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0117.jpg',
            'descripcion'=>'',
            'production_id'=>'4',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0090.jpg',
            'descripcion'=>'',
            'production_id'=>'4',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0083.jpg',
            'descripcion'=>'',
            'production_id'=>'4',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0078.jpg',
            'descripcion'=>'',
            'production_id'=>'4',
        ]);
        DB::table('images')->insert([
            'link' => 'DSC_0068.jpg',
            'descripcion'=>'',
            'production_id'=>'4',
        ]);
        DB::table('images')->insert([
            'link' => '13886289_605532339627221_3783659007067330042_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '14324532_623800294467092_5640432602000664904_o.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '14379599_624344154412706_120796795583453766_o.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '14524477_630877983759323_2703754558259932702_o.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '13903295_609346635912458_2709144926610823815_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '13925067_608080566039065_4492183010568809425_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '13906644_608521762661612_1307048138247583387_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '13900311_608521805994941_8206540506265586204_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '13938361_609346689245786_5751593897211626939_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => '14021624_609598049220650_2981885948322427594_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        
        DB::table('images')->insert([
            'link' => '14330098_625755510938237_635355186869108159_n.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        
        DB::table('images')->insert([
            'link' => 'cartel.jpg',
            'descripcion'=>'',
            'proyect_id'=>'1',
        ]);
        DB::table('images')->insert([
            'link' => 'lamujer1.jpg',
            'descripcion'=>'',
            'production_id'=>'3',
        ]);
        DB::table('images')->insert([
            'link' => 'lamujer2.jpg',
            'descripcion'=>'',
            'production_id'=>'3',
        ]);
        DB::table('images')->insert([
            'link' => 'lamujer3.jpg',
            'descripcion'=>'',
            'production_id'=>'3',
        ]);
    }
}
